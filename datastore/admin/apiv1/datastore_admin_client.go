...
func defaultDatastoreAdminCallOptions() *DatastoreAdminCallOptions {
	...
	return &DatastoreAdminCallOptions{
		ExportEntities: retry[[2]string{"default", "non_idempotent"}],
		ImportEntities: retry[[2]string{"default", "non_idempotent"}],
...
func NewDatastoreAdminClient(ctx context.Context, opts ...option.ClientOption) (*DatastoreAdminClient, error) {
	conn, err := transport.DialGRPC(ctx, append(defaultDatastoreAdminClientOptions(), opts...)...)
	if err != nil {
		return nil, err
	}
	c := &DatastoreAdminClient{
		...
		datastoreAdminClient: adminpb.NewDatastoreAdminClient(conn),
	}
